...
COMMON_JS_RUNTIME_PATHS = {
    "node": [
        Path.home() / ".nvm/versions/node/current/bin/node",
        Path.home() / ".nvm/versions/node/v20.19.0/bin/node",
...
class YouTubeDownloader:
    ...
    def _resolve_js_runtime(self) -> Optional[Dict[str, str]]:
        ...
        for runtime in runtime_candidates:
            ...
            for candidate in COMMON_JS_RUNTIME_PATHS.get(runtime, []):
                if candidate.is_file():
                    logger.info(f"🧩 Found {runtime} runtime outside PATH: {candidate}")
                    return {'runtime': runtime, 'path': str(candidate)}
    ...
    def _get_format_selector(self) -> str:
        ...
        if self.quality == "best":
            # Prefer progressive H.264 MP4 first, then fall back to DASH pairs.
            return (
